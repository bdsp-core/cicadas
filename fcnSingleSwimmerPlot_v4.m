...
% Add colorbar for L
% Create a dummy axes for colorbar
c1 = colorbar;
% Create colormap for colorbar only
n_colors = 256;
cmap_L = zeros(n_colors, 3);
for i = 1:n_colors
    intensity = sqrt((i-1) / (n_colors-1));
    % Light red to Dark red colormap
    cmap_L(i, :) = [1 - 0.5*intensity, 0.8*(1-intensity), 0.8*(1-intensity)];
end
colormap(ax1, cmap_L);
clim([0 max_L]);
...
for i = 1:n_colors
    intensity = sqrt((i-1) / (n_colors-1));
    % Light blue to Dark blue colormap
    cmap_A(i, :) = [0.8*(1-intensity), 0.8*(1-intensity), 1 - 0.5*intensity];
end
colormap(ax2, cmap_A);
clim([0 max_A]);
