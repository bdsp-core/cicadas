...
function [t, z, t1, t2, p, L, f, A, V, Y,hY,SY,hA,SA,hV,SV] = fcnRunSimulation_v2(Rx, harmE, harmA, C, g, th, kp, ki, Amax, b0, pulseAmp, pulseMu, pulseWidth, pulseC,sim_mode) 
 ...
 for j = 2:Nt
     ...
     % ---- Treatment stopping logic (NEW) ---------------------------
     % Probability of stopping depends on L: low L -> higher chance of stopping
     if sim_mode == 1 && stopped_treatment == 0 && currentRx == 1
     ...
     if sim_mode == 1 && switchTime == 0 && stopped_treatment == 0 && rand < switch_hazard * dt
     ...
     % Determine if death occurs (only in simulation mode)
     if sim_mode == 1 && rand < hY(j) * dt
 ...
 % In non-simulation mode, keep everyone in the trial, alive, and no treatment changes
 if sim_mode == 0
     V = zeros(1,Nt);
     Y = zeros(1,Nt);
