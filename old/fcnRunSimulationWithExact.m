%% Modified fcnRunSimulation - Now with exact hazard and survival functions - FIXED VERSION
function [t, L, A, V, Y, exact_hY, exact_SY, exact_hV, exact_SV, exact_hA, exact_SA, t_exact] = fcnRunSimulationWithExact(Rx, harmE, harmA, C, g, th, kp, ki, Amax, b0, pulseAmp, pulseMu, pulseWidth, pulseC, RCT) 
 ...
 % Exact hazard functions (analytical)
 ...
 % Exact cumulative survival probabilities (analytical)
 exact_SV = ones(1,Nt); % Exact probability of not having dropped out
 exact_SY = ones(1,Nt); % Exact probability of not having died
 exact_SA = ones(1,Nt); % Exact probability of not having switched treatment
 ...
 for j = 2:Nt
     ...
     % ---- EXACT hazard functions and survival curves (analytical) ----
     % These are calculated for all time points, regardless of death/dropout in simulation
     
     % Exact dropout hazard
     exact_hV(j) = dropout_hazard_base * (1 + 2*L(j)^2 + 0.5*(harmE + harmA)+10*((j/Nt)^2));
     exact_SV(j) = exact_SV(j-1) * exp(-exact_hV(j) * dt);
     
     % Exact switching hazard
     exact_hA(j) = switch_hazard_max * exp(-switch_hazard_decay * t(j));
     exact_SA(j) = exact_SA(j-1) * exp(-exact_hA(j) * dt);
     
     % Exact mortality hazard
     exact_hY(j) = t1(j) + t2(j);
 ...
 % Save the full time vector for exact hazard functions before truncating
 t_exact = t;
 ...
 if sum(Y)>0; 
     ...
     hV=hV(ii); 
     SV=SV(ii); 
     % Exact hazards and survivals are NOT truncated
     % as they represent the theoretical functions
 end
 if sum(V)>0; 
     ind = find(V==1); 
     ii = 1:min(ind); 
